...
int IntPow(int base, int exponent) {
	int i;
	int output = 1;
	for (i = 0; i < exponent; i++) { output *= base; }
	return output;
}
...
void BMIH::display(void) {
	using namespace std;
	cout << "biSize: " << static_cast<int>(biSize) << endl
		<< "biWidth: " << static_cast<int>(biWidth) << endl
		<< "biHeight: " << static_cast<int>(biHeight) << endl
		<< "biPlanes: " << static_cast<int>(biPlanes) << endl
		<< "biBitCount: " << static_cast<int>(biBitCount) << endl
		<< "biCompression: " << static_cast<int>(biCompression) << endl
		<< "biSizeImage: " << static_cast<int>(biSizeImage) << endl
		<< "biXPelsPerMeter: " << static_cast<int>(biXPelsPerMeter) << endl
		<< "biYPelsPerMeter: " << static_cast<int>(biYPelsPerMeter) << endl
		<< "biClrUsed: " << static_cast<int>(biClrUsed) << endl
		<< "biClrImportant: " << static_cast<int>(biClrImportant) << endl << endl;
}

void BMFH::display(void) {
	using namespace std;
	cout << "bfType: " << static_cast<int>(bfType) << endl
		<< "bfSize: " << static_cast<int>(bfSize) << endl
		<< "bfReserved1: " << static_cast<int>(bfReserved1) << endl
		<< "bfReserved2: " << static_cast<int>(bfReserved2) << endl
		<< "bfOffBits: " << static_cast<int>(bfOffBits) << endl << endl;
}
...
BMP::BMP(BMP& Input) {
	...
	if (BitDepth == 1 || BitDepth == 4 ||
		BitDepth == 8) {
		for (int k = 0; k < TellNumberOfColors(); k++) { SetColor(k, Input.GetColor(k)); }
	}
...
bool BMP::SetBitDepth(int NewDepth) {
	...
	if (BitDepth == 1 || BitDepth == 4 || BitDepth == 8) { CreateStandardColorTable(); }
...
bool BMP::SetSize(int NewWidth, int NewHeight) {
	...
	for (i = 0; i < Width; i++) { Pixels[i] = new RGBApixel [ Height ]; }
...
bool BMP::WriteToFile(const char* FileName) {
	...
	BMFH bmfh;
	bmfh.bfSize = static_cast<ebmpDWORD>(dTotalFileSize);
	bmfh.bfReserved1 = 0;
	bmfh.bfReserved2 = 0;
	bmfh.bfOffBits = static_cast<ebmpDWORD>(14 + 40 + dPaletteSize);
	...
	bmih.biSizeImage = static_cast<ebmpDWORD>(dTotalPixelBytes);
	...
	if (BitDepth == 1 || BitDepth == 4 || BitDepth == 8) {
		...
		if (!Colors) {
			if (!Colors) { Colors = new RGBApixel [NumberOfColors]; }
			CreateStandardColorTable();
		}

		int n;
		for (n = 0; n < NumberOfColors; n++) { fwrite((char*)&(Colors[n]), 4, 1, fp); }
	}
	...
	if (BitDepth != 16) {
		...
		while (j > -1) {
			...
			if (Success) {
				int BytesWritten = static_cast<int>(fwrite((char*)Buffer, 1, BufferSize, fp));
	...
	if (BitDepth == 16) {
		...
		for (j = Height - 1; j >= 0; j--) {
			...
			while (WriteNumber < DataBytes) {
				...
				ebmpWORD RedWORD = static_cast<ebmpWORD>((Pixels[i][j]).Red / 8);
				ebmpWORD GreenWORD = static_cast<ebmpWORD>((Pixels[i][j]).Green / 4);
				ebmpWORD BlueWORD = static_cast<ebmpWORD>((Pixels[i][j]).Blue / 8);
...
bool BMP::ReadFromFile(const char* FileName) {
	...
	if (!IsBigEndian() && bmfh.bfType == 19778) { IsBitmap = true; }
	...
	int TempBitDepth = static_cast<int>(bmih.biBitCount);
	...
	SetBitDepth(static_cast<int>(bmih.biBitCount));
	...
	if (BitDepth < 16) {
		...
		int n;
		for (n = 0; n < NumberOfColorsToRead; n++) { SafeFread((char*)&(Colors[n]), 4, 1, fp); }
	...
	if (BitDepth != 16) {
		...
		j = Height - 1;
		while (j > -1) {
			int BytesRead = static_cast<int>(fread((char*)Buffer, 1, BufferSize, fp));
...
bool BMP::ReadFromBuffer(const char* buffer, const size_t length) {
	using namespace std;
	if (!EasyBMPcheckDataSize()) {
		if (EasyBMPwarnings) {
			cout << "EasyBMP Error: Data types are wrong size!" << endl
	...
	DataArray *ap = new DataArray{};
	ap->buffer = (uint8_t *) buffer;
	ap->size = length;
	ap->offset = 0;
	...
	NotCorrupted &= SafeAread((char*)&(bmfh.bfType), sizeof(ebmpWORD), 1, ap);
	...
	NotCorrupted &= SafeAread((char*)&(bmfh.bfSize), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmfh.bfReserved1), sizeof(ebmpWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmfh.bfReserved2), sizeof(ebmpWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmfh.bfOffBits), sizeof(ebmpDWORD), 1, ap);
	...
	NotCorrupted &= SafeAread((char*)&(bmih.biSize), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biWidth), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biHeight), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biPlanes), sizeof(ebmpWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biBitCount), sizeof(ebmpWORD), 1, ap);

	NotCorrupted &= SafeAread((char*)&(bmih.biCompression), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biSizeImage), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biXPelsPerMeter), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biYPelsPerMeter), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biClrUsed), sizeof(ebmpDWORD), 1, ap);
	NotCorrupted &= SafeAread((char*)&(bmih.biClrImportant), sizeof(ebmpDWORD), 1, ap);
	...
	if (BitDepth < 16) {
		...
		int n;
		for (n = 0; n < NumberOfColorsToRead; n++) { SafeAread((char*)&(Colors[n]), 4, 1, ap); }
		for (n = NumberOfColorsToRead; n < TellNumberOfColors(); n++) {
			RGBApixel WHITE;
			WHITE.Red = 255;
			WHITE.Green = 255;
	...
	if (BytesToSkip > 0 && BitDepth != 16) {
		ebmpBYTE* TempSkipBYTE;
		TempSkipBYTE = new ebmpBYTE [BytesToSkip];
		SafeAread((char*)TempSkipBYTE, BytesToSkip, 1, ap);
		delete [] TempSkipBYTE;
	}
	...
	if (BitDepth != 16) {
		...
		j = Height - 1;
		while (j > -1) {
			int BytesRead = static_cast<int>(aread((char*)Buffer, 1, BufferSize, ap));
			if (BytesRead < BufferSize) {
				j = -1;
			}
			else {
	...
	if (BitDepth == 16) {
		...
		if (bmih.biCompression != 0) {
			...
			SafeAread((char*)&RedMask, 2, 1, ap);
			if (IsBigEndian()) { RedMask = FlipWORD(RedMask); }
			SafeAread((char*)&TempMaskWORD, 2, 1, ap);

			SafeAread((char*)&GreenMask, 2, 1, ap);
			if (IsBigEndian()) { GreenMask = FlipWORD(GreenMask); }
			SafeAread((char*)&TempMaskWORD, 2, 1, ap);

			SafeAread((char*)&BlueMask, 2, 1, ap);
			if (IsBigEndian()) { BlueMask = FlipWORD(BlueMask); }
			SafeAread((char*)&TempMaskWORD, 2, 1, ap);
		}
		...
		for (j = Height - 1; j >= 0; j--) {
			...
			while (ReadNumber < DataBytes) {
				ebmpWORD TempWORD;
				SafeAread((char*)&TempWORD, 2, 1, ap);
				if (IsBigEndian()) { TempWORD = FlipWORD(TempWORD); }
				ReadNumber += 2;
			...
			while (ReadNumber < PaddingBytes) {
				ebmpBYTE TempBYTE;
				SafeAread((char*)&TempBYTE, 1, 1, ap);
				ReadNumber++;
			}
		}
	}
...
bool SafeFread(char* buffer, int size, int number, FILE* fp) {
	...
	ItemsRead = static_cast<int>(fread(buffer, size, number, fp));
	if (ItemsRead < number) { return false; }
	return true;
}

bool SafeAread(char* buffer, int size, int number, DataArray* ap) {
	using namespace std;
	int ItemsRead = size * number;
	if (ap->offset + ItemsRead >= ap->size) { return false; }
	memcpy(buffer, ap->buffer + ap->offset, ItemsRead);
	ap->offset += ItemsRead;
	return true;
}

int aread(char* buffer, int size, int number, DataArray* ap) {
	using namespace std;
	int ItemsRead = min(size * number, ap->size - ap->offset);
	memcpy(buffer, ap->buffer + ap->offset, ItemsRead);
	ap->offset += ItemsRead;
	return ItemsRead;
}
...
int BMP::TellVerticalDPI(void) {
	if (!YPelsPerMeter) { YPelsPerMeter = DefaultYPelsPerMeter; }
	return static_cast<int>(YPelsPerMeter / static_cast<double>(39.37007874015748));
}

// int BMP::TellHorizontalDPI( void ) const
int BMP::TellHorizontalDPI(void) {
	if (!XPelsPerMeter) { XPelsPerMeter = DefaultXPelsPerMeter; }
	return static_cast<int>(XPelsPerMeter / static_cast<double>(39.37007874015748));
}
...
BMIH GetBMIH(const char* szFileNameIn) {
	...
	ebmpBYTE TempBYTE;
	int i;
	for (i = 14; i > 0; i--) { SafeFread((char*)&TempBYTE, sizeof(ebmpBYTE), 1, fp); }
...
int GetBitmapColorDepth(const char* szFileNameIn) {
	BMIH bmih = GetBMIH(szFileNameIn);
	return static_cast<int>(bmih.biBitCount);
}

void PixelToPixelCopy(BMP& From, int FromX, int FromY,
                      BMP& To, int ToX, int ToY) { *To(ToX, ToY) = *From(FromX, FromY); }
...
void PixelToPixelCopyTransparent(BMP& From, int FromX, int FromY,
	...
	if (From(FromX, FromY)->Red != Transparent.Red ||
		...
		From(FromX, FromY)->Blue != Transparent.Blue) { *To(ToX, ToY) = *From(FromX, FromY); }
}
...
bool CreateGrayscaleColorTable(BMP& InputImage) {
	...
	ebmpBYTE StepSize;
	if (BitDepth != 1) { StepSize = 255 / (NumberOfColors - 1); }
	else { StepSize = 255; }
...
bool BMP::Read32bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
	...
	for (i = 0; i < Width; i++) { memcpy((char*)&(Pixels[i][Row]), (char*)Buffer + 4 * i, 4); }
	return true;
}
...
bool BMP::Read4bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
	...
	while (i < Width) {
		j = 0;
		while (j < 2 && i < Width) {
			int Index = static_cast<int>((Buffer[k] & Masks[j]) >> Shifts[j]);
			*(this->operator()(i, Row)) = GetColor(Index);
			i++;
			j++;
		}
...
bool BMP::Write32bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
	int i;
	if (Width * 4 > BufferSize) { return false; }
	for (i = 0; i < Width; i++) { memcpy((char*)Buffer + 4 * i, (char*)&(Pixels[i][Row]), 4); }
	return true;
}
...
bool BMP::Write24bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
	int i;
	if (Width * 3 > BufferSize) { return false; }
	for (i = 0; i < Width; i++) { memcpy((char*)Buffer + 3 * i, (char*)&(Pixels[i][Row]), 3); }
	return true;
}
...
bool BMP::Write8bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
	int i;
	if (Width > BufferSize) { return false; }
	for (i = 0; i < Width; i++) { Buffer[i] = FindClosestColor(Pixels[i][Row]); }
	return true;
}
...
bool BMP::Write4bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
	...
	while (i < Width) {
		...
		int Index = 0;
		while (j < 2 && i < Width) {
			Index += (PositionWeights[j] * static_cast<int>(FindClosestColor(Pixels[i][Row])));
			i++;
			j++;
		}
		Buffer[k] = static_cast<ebmpBYTE>(Index);
		k++;
	}
	return true;
}
...
ebmpBYTE BMP::FindClosestColor(RGBApixel& input) {
	...
	while (i < NumberOfColors) {
		...
		int TempMatch = IntSquare(static_cast<int>(Attempt.Red) - static_cast<int>(input.Red))
			+ IntSquare(static_cast<int>(Attempt.Green) - static_cast<int>(input.Green))
			+ IntSquare(static_cast<int>(Attempt.Blue) - static_cast<int>(input.Blue));
		if (TempMatch < BestMatch) {
			BestI = static_cast<ebmpBYTE>(i);
			BestMatch = TempMatch;
		}
...
bool Rescale(BMP& InputImage, char mode, int NewDimension) {
	...
	if (CapMode == 'F') {
		if (OldWidth > OldHeight) { CapMode = 'W'; }
		else { CapMode = 'H'; }
	}
	...
	for (int j = 0; j < NewHeight - 1; j++) {
		ThetaJ = static_cast<double>(j * (OldHeight - 1.0))
			/ static_cast<double>(NewHeight - 1.0);
		J = static_cast<int>(floor(ThetaJ));
		ThetaJ -= J;

		for (int i = 0; i < NewWidth - 1; i++) {
			ThetaI = static_cast<double>(i * (OldWidth - 1.0))
				/ static_cast<double>(NewWidth - 1.0);
			I = static_cast<int>(floor(ThetaI));
			ThetaI -= I;

			InputImage(i, j)->Red = static_cast<ebmpBYTE>((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ)
				* (OldImage(I, J)->Red)
				+ (ThetaI - ThetaI * ThetaJ) * (OldImage(I + 1, J)->Red)
				+ (ThetaJ - ThetaI * ThetaJ) * (OldImage(I, J + 1)->Red)
				+ (ThetaI * ThetaJ) * (OldImage(I + 1, J + 1)->Red));
			InputImage(i, j)->Green = static_cast<ebmpBYTE>((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ
				) * OldImage(I, J)->Green
				+ (ThetaI - ThetaI * ThetaJ) * OldImage(I + 1, J)->Green
				+ (ThetaJ - ThetaI * ThetaJ) * OldImage(I, J + 1)->Green
				+ (ThetaI * ThetaJ) * OldImage(I + 1, J + 1)->Green);
			InputImage(i, j)->Blue = static_cast<ebmpBYTE>((1.0 - ThetaI - ThetaJ + ThetaI * ThetaJ)
				* OldImage(I, J)->Blue
				+ (ThetaI - ThetaI * ThetaJ) * OldImage(I + 1, J)->Blue
				+ (ThetaJ - ThetaI * ThetaJ) * OldImage(I, J + 1)->Blue
				+ (ThetaI * ThetaJ) * OldImage(I + 1, J + 1)->Blue);
		}
		InputImage(NewWidth - 1, j)->Red = static_cast<ebmpBYTE>((1.0 - ThetaJ) * (OldImage(
				OldWidth - 1, J)->Red)
			+ ThetaJ * (OldImage(OldWidth - 1, J + 1)->Red));
		InputImage(NewWidth - 1, j)->Green = static_cast<ebmpBYTE>((1.0 - ThetaJ) * (OldImage(
				OldWidth - 1, J)->Green)
			+ ThetaJ * (OldImage(OldWidth - 1, J + 1)->Green));
		InputImage(NewWidth - 1, j)->Blue = static_cast<ebmpBYTE>((1.0 - ThetaJ) * (OldImage(
				OldWidth - 1, J)->Blue)
	...
	for (int i = 0; i < NewWidth - 1; i++) {
		...
		I = static_cast<int>(floor(ThetaI));
		ThetaI -= I;
		InputImage(i, NewHeight - 1)->Red = static_cast<ebmpBYTE>((1.0 - ThetaI) * (OldImage(
				I, OldHeight - 1)->Red)
			+ ThetaI * (OldImage(I, OldHeight - 1)->Red));
		InputImage(i, NewHeight - 1)->Green = static_cast<ebmpBYTE>((1.0 - ThetaI) * (OldImage(
				I, OldHeight - 1)->Green)
			+ ThetaI * (OldImage(I, OldHeight - 1)->Green));
		InputImage(i, NewHeight - 1)->Blue = static_cast<ebmpBYTE>((1.0 - ThetaI) * (OldImage(
				I, OldHeight - 1)->Blue)
